...
print(Fore.BLUE + Style.BRIGHT + custom_fig.renderText('Django-Debug-ModeEnabled') + Style.RESET_ALL)
...
class DjangoDebugModeList:
    def __init__(self,domainList,Shodankey):
        ...
        self.django_debug_list = [] # list to store django_debug_list # this What I return
        self.domainIp = {} # to understand Which IP belongs to which Domain
    def return_domain_List(self):
        return self.domainIp

    def DjangoList(self): # returns the list
        ...
        for word in self.domainList:
            ...
            try:
                ...
                for result in results['matches']:
                    ...
                    self.django_debug_list.append(ip)
                    

            except Exception as e:
               print(e)
        domainIpFile.write(json.dumps(self.domainIp))
        return self.django_debug_list 
...
class AutomationAndReporting():
    def __init__(self):
        ...
        self.IpFinalList = []
    def IpChecking(self,django_debug_list): # this list is ==> djangoList.DjangoList() 
        regex = r"(?:mongodb|redis):\/\/"

        for ip in django_debug_list:
    ...
    def reportMaking(self,domainAndIp): #domainAndIp -->DjangoDebugModeList.return_domain_List()
        fileName = "FinalResponse.txt"
        path = os.path.join(os.getcwd(),fileName)
        FinalResponse =  open(path, "w") 

        IpAddressDummmy = self.IpChecking(djangoList.DjangoList())
...
djangoList = DjangoDebugModeList(argument.returnWordList(),argument.returnShodanKey())
automatinoOne = AutomationAndReporting()
#all=automatinoOne.IpChecking(djangoList.DjangoList())
reportMakingOne = automatinoOne.reportMaking(djangoList.return_domain_List())
print(all)
